...
| Signal direction | **Empirical, never assumed** — v0 no-trade baseline (logs every trigger), v1 exhaustion-confirmed mean-reversion (first live), v2 momentum, v3 flow-classifying hybrid router (end-state target) | Direction decided by out-of-sample evidence + live shadow on the same event. M26: shadow now loads real `tick_aggregates` for the signal bar (M7 backtest pattern) and aligns entry to the next-bar open for same-tape parity, producing virtual PnL for counterfactual comparison (ADR 0029). Close-side still uses reference-price proxy; forward-only ledger from deploy. |
...
- **RiskModule** — gatekeeper: sizing, daily/weekly loss limits, **consecutive-loss / per-symbol / per-bar caps**, exposure caps, liquidity/funding filters, **global market-stress halt** (driven by the fast-stress inputs, overrides ADX), **model-divergence kill switch**, stop-loss (ATR or structural) + take-profit, cooldowns. Signal → approved/rejected order intent. *M19: book depth is a **per-coin** eligibility skip (`coin_book_too_thin`, per-tier floors), NOT a global halt — a thin alt can no longer day-kill the whole market; the global stress halt now covers spread-widening + breadth-distance (risk-only const, decoupled from the `stress_breadth_pct` flow param) + OI/funding/index. See ADR 0004 §6/§6a/§6b. M21: BTC and ETH index-shock legs aligned to 5-minute horizon: `STRESS_BTC_5M_SHOCK_PCT = 1.5` (BTC 1m was inert, peak 0.56%), `STRESS_ETH_5M_SHOCK_PCT = 2.5` (raised from 2.0; only event was 2.12%). Atomic `hasInvalidStressInputs` swap. See ADR 0004 §6c. M22: `COIN_DEPTH_FLOOR_10BPS_USDT` recalibrated from M19 round numbers to book-consumption-ratio anchors: `{ TIER_1: 10_000, TIER_2: 2_500, TIER_3: 2_000 }` (one-sided at $250 max order). Soak evidence: 7 unblocked at $3,468–$9,174; 3 still blocked at $529/$681/$2,321. See ADR 0004 §6a (14-day post-deploy slippage telemetry required as calibration condition). M23: breadth-triggered `market_stress` halts now auto-resume after `MARKET_STRESS_RESUME_CLEAR_TICKS=3` consecutive clean ticks in the hysteresis band (distance ≤30), with per-day re-halt cap (3 → full-day lock). `halt_reason` extended to `market_stress:<leg>` suffix (`:breadth` resume-eligible, others full-day locked). No migration. Paper-only initially (code-enforced via `MARKET_STRESS_AUTO_RESUME_ENABLED` boot flag); live activation gated on backtest + 14-day soak. See ADR 0004 §6d. M25: paper-only exploration profile exists (ADR 0042) — stress legs relaxable per `PAPER_RELAX_MARKET_STRESS=true` when `EXCHANGE_ENV=paper`, sizing headroom env-driven, 3-slot ceiling unchanged in all envs (true 5-slot expansion deferred to follow-on). Invalid-inputs guard + breadth always remain protected even in paper. See ADR 0042.*
