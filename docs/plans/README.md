...
|----|--------|------------------|------|---------|
| M52 | PLANNED | xmom `force_close` slot recovery — bounded, volatility-gated **same-coin** retry (ADR 0051). When the ADR 0045 fill-acceptance guard `force_close`'s a scheduled-rebalance momentum OPEN (async, after the cascade's `rebalance()` returns → slot left empty until the next 24h cron), the orchestrator MAY re-attempt the **same** coin — but only when a circuit breaker keyed on the already-logged `atrUnits` drift says the drift was a plausibly-**stale reference** (provisional < 1.0 ATR), not a genuine dislocation (observed 2026-07-03: FARTCOIN 1.76 / WLD 1.32 → both leave slot empty, intended). Retry is cooldown-deferred to the next closed 5m bar (re-anchored, never instant), freshly sized (never reuse attempt-1 notional), reservation-safe (fires after the unwind's `POSITION_CLOSED_EVENT` release), **paper-only + default-off** (`XMOM_FORCE_CLOSE_RETRY`). **Not a bug fix** — the guard works and is cheap (~$0.20 unwind vs ~$14.91 stop_loss); this closes the empty-slot gap. **D1** breaker + per-cycle attempt ledger (`rebalanceCycleId`, `MOMENTUM_FILL_FORCE_CLOSED_EVENT`, live-`top_n` re-check); **D2** cooldown/re-anchor execution path behind the flag; **D3** soak observability (retry fill-accept rate, retry-vs-attempt-1 forward returns = decisive adverse-selection test, `atrUnits` drift distribution for data-driven threshold calibration, slippage, counterfactual PnL) + new `EXP-0xx`; **D4** go/no-go closure. **Non-goals (rejected — do not re-propose):** no next-ranked-coin backfill, no faster cron, no guard weakening. Guard/gate/core untouched; inert in backtest; inherits xmom HIGH go-live blockers. | 0051, 0045, 0048, 0050, 0004 | strategy, execution, risk, analysis, paper-mode |
