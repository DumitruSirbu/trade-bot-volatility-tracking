...
|----|--------|------------------|------|---------|
| M51 | PLANNED | Unblock the xmom PAPER soak — 0 fills in 185 gate attempts across two soak eras. **D1 (P0, blocking):** align the momentum time-stop gate ceiling with the intent's intentional 2× rebalance-interval margin — `buildGateStrategyParams` derives `time_stop_minutes` from a straight 1× while the intent proposes 2×, so `RiskGateService.checkTimeStop` rejects every deep-book symbol `time_stop_missing_or_invalid` (84% of the 01:07 cron run); centralize the 2× in one constant so intent + ceiling can't drift. **D2 (P1, blocking):** paper-only per-coin liquidity relax behind default-off `PAPER_RELAX_PER_COIN_LIQUIDITY` (depth >$2,500, spread ≤0.30%) so thin momentum leaders (ranks 1–7) clear the gate in PAPER — never touches live tier1 floors, unreachable on LIVE. **D3/D4 stretch:** `depth=0` feed-artifact investigation (keep fail-closed), pre-gate-skip visibility. **D5 docs:** manual-trigger (`pnpm rebalance:trigger`) is the sanctioned tuning-visibility tool — cadence stays fixed 24h cron (hourly rejected: noise-trades a 24h signal + fee churn). No live floors, no cadence change, no signal change, no live capital. Implements `docs/wip/2026-07-01-xmom-paper-liquidity-gate-analysis.md`. | 0048, 0042, 0004 | risk, strategy, config |
