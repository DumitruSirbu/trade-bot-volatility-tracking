...
|----|--------|------------------|------|---------|
| M50 | PLANNED | Cross-sectional momentum as a NEW parallel strategy (xmom) — rank the universe by trailing 24h return, hold the top-ranked winner(s) 24h, re-rank on a 24h cadence (EXP-011/012). PAPER + shadow only this milestone, NO live capital (EXP-012 post-cost t<2 on one up-regime; live promotion gated on a down-regime soak). New pure `IPortfolioStrategy` contract (distinct from single-symbol `IStrategy`, OCP) + `crossSectionalMomentumCore` ranking fn; `RebalanceSchedulerService` emits `UNIVERSE_REBALANCE_DUE_EVENT{nowMs}` (deterministic clock); `MomentumOrchestratorService` routes each selected leg through the **unchanged** risk gate / `PositionSizer` / execution path. Single-slot long-only proxy (`top_n=1`) fits the existing 3-slot model — no cap change; the N-long basket (per-strategy position + notional cap, separate slot namespace) is designed (D4) but DEFERRED to M50b. Separate `ACTIVE_PORTFOLIO_STRATEGY_VERSION_ID` env + disjoint orchestrators run momentum alongside live VWAP, but they SHARE the global A/B/C slot pool in M50 (separate namespace is the M50b feature) — momentum rebalances can gate-reject when VWAP holds all slots. Separate `momentumParamsSchema` (the VWAP schema is `.strict()`). Reconciliation already per-`(symbol,side)` → multi-position needs no change. VWAP path (v0–v3) untouched. New ADRs 0047 (portfolio-strategy contract) + 0048 (rebalance orchestrator); ADR 0004 deferred note for the momentum cap model. | 0047, 0048, 0004, 0003 | strategy, risk, shared, market-data, paper-mode |
