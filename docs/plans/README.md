...
|----|--------|------------------|------|---------|
| M53 | DONE | xmom take-profit **arm headroom** (decouple arm ratio from guard floor) — **D1 + D4 shipped, D2/D3 deferred to follow-up.** **D1 (shipped, no-op default):** new param `xmom_tp_arm_rr` defaults 1.5 on active version (`id=20`); wired at arm site only, `xmom_min_rr` guard-floor thread unchanged; byte-for-byte no-op. **D4 (shipped, migration pending DB execution):** forward-only migration archives 7 dead `volatility-vwap` shadow rows (ids 1,2,4,15,16,17,19); ⚠️ **pg_dump + user confirm required (CLAUDE.md rule-9) before running on real DB.** **D2/D3 deferred:** multi-ratio shadow cohort + per-cohort instrumentation depend on architect feasibility spike (route 1 cascade fan-out vs route 2 offline replay); portfolio-strategy shadow path not on existing single-symbol VWAP engine — spike must pass gate (a) realistic entry slippage + gate (b) 1.5-cohort reproduces live reality before cohort rows seeded. EXP-019 NOT registered (D3 didn't ship). Tests: 21 adversarial (all passing). Review: zero blockers/highs. ADRs: 0047 amended (new param, decoupling); 0045/0050/0051 referenced. | 0047, 0045, 0050, 0051, 0029 | strategy, shared, shadow, analysis |
| M52 | PLANNED | xmom `force_close` slot recovery — bounded, volatility-gated **same-coin** retry (ADR 0051). **D1–D3 SHIPPED/REVIEWED (2026-07-03):** D1 circuit breaker keyed on `atrUnits` drift (provisional 1.0) + per-cycle attempt ledger (`rebalanceCycleId`, `MOMENTUM_FILL_FORCE_CLOSED_EVENT`, live-`top_n` re-check); D2 cooldown/re-anchor execution path behind default-off paper-only `XMOM_FORCE_CLOSE_RETRY`; D3 soak observability (retry fill-accept rate, retry-vs-attempt-1 forward returns = decisive adverse-selection test, `atrUnits` drift distribution for calibration, slippage, counterfactual PnL, `EXP-017` registered). Code deployed to live paper-soak container with flag active. **D4 soak-gate closure PENDING:** requires real force_close + retry events to evaluate ADR 0051 §6 go/no-go bar (adverse-selection, survival rate, PnL, threshold calibration). **Not a bug fix** — the ADR 0045 guard works and is cheap (~$0.20 unwind vs ~$14.91 stop_loss); M52 only recovers the empty-slot gap. Guard/gate/core untouched; inert in backtest; inherits xmom HIGH go-live blockers. **Non-goals (rejected — do not re-propose):** no next-ranked-coin backfill, no faster cron, no guard weakening. | 0051, 0045, 0048, 0050, 0004 | strategy, execution, risk, analysis, paper-mode |
| M51 | DONE | Unblock xmom PAPER soak (0→3 gate approvals per cycle). **D1 (P0, blocking, shipped):** time-stop gate-ceiling alignment — new constant `MOMENTUM_TIME_STOP_MARGIN_MULTIPLIER = 2` in `strategyConsts.ts`; both intent's `timeStopAtMs` and `buildGateStrategyParams`'s `time_stop_minutes` ceiling derive from it (previously ceiling used 1×, intent 2×, rejecting 84% of the cron run on `time_stop_missing_or_invalid`). ADR 0048 amended. **D2 (P1, blocking, shipped):** paper-only per-coin liquidity relax behind default-off `PAPER_RELAX_PER_COIN_LIQUIDITY` (depth >$2,500 / spread ≤0.30%), combined per-tier via `Math.min`/`Math.max` with live tier floors (not a flat override — preserves tier3 floors). QA found and fixed tier3 regression: tier1/2 get intended relax, tier3 reverts to live floor. ADR 0042 §9 amended (new section), ADR 0004 §6a additive pointer. **D3/D4 (stretch, deferred):** `depth=0` feed-artifact investigation (keep fail-closed), pre-gate-skip visibility remain open; noted in tech-debt. **Smoke test (2026-07-02):** 3 gate approvals + 3 thin-book rejects in one cycle — first real end-to-end lifecycle in 185 prior gate attempts with 0 fills. TLM + ALLO opened and held; RE opened then flattened by pre-existing `degenerate_geometry_at_fill` guard (working as designed). Cold-boot caveat: `book_depth_10bps_usdt` reads 0 until depth wiring organically captures real data (expected, matches plan deferred D3 note). Quant review: 1 High (confirmed paper fill simulator fills flat at best-quote with zero slippage; annotate M51-era PnL as pipeline-validation-only when any PnL dashboard exists), 2 Mediums (one fixed: boot warning when `PAPER_RELAX_PER_COIN_LIQUIDITY` active + `MAX_EXPOSURE_PER_COIN_USDT > $500`; one accepted: paper/live liquidity-universe asymmetry is documented design tradeoff). Branch: `fix/m51-xmom-paper-gate-unblock`. | 0048, 0042, 0004 | risk, strategy, config |
| M50 | PLANNED | Cross-sectional momentum (xmom) — PAPER + shadow only. **M50b increment DONE (2026-07-01, ADR 0050):** rank-cascade in orchestrator, `top_n` default 3, fixed 01:07 UTC cron; `IPortfolioSelection.ranked` full list. **M50c increment DONE (2026-07-02, ADR 0048 Amendment):** manual-rebalance trigger endpoint (`REST` + `CLI`), `positions.trigger_source` attribution column (migration `20260710000000`), analysis exclusion filters for manual trades. Core M50 soak/promotion gates still open. Original brief: rank universe by trailing 24h return, 24h cadence (EXP-011/012). `IPortfolioStrategy` + `crossSectionalMomentumCore`; `RebalanceSchedulerService` + `MomentumOrchestratorService` through unchanged risk gate. Disjoint slot namespace (D4) still deferred. | 0047, 0048, 0050, 0004, 0003 | strategy, risk, shared, market-data, paper-mode |
