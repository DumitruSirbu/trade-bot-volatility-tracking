...
- **ADRs 0033 + 0034 Accepted-and-shipped:** ADR 0033 (MCP module-boundary enforcement: workspace deps, ESLint rule, Layer A/B/C defense-in-depth). ADR 0034 (MCP DB isolation: read-only role, 30s timeout, 3-connection pool, per-tool budgets). Both reference `docs/plans/M12-analysis-mcp.md` + `M12-execution-plan.md` + operator runbook.
