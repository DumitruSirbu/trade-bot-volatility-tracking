...
| [0047 § M54 amendment](0047-portfolio-strategy-contract.md#7-m54-amendment-2026-07-09--expected-fill-anchor--thin-book-skip) | M54 — `xmom_expected_fill_enabled` + `xmom_max_depth_fraction` arm SL/TP off the expected fill price + pre-send thin-book skip (both no-op at defaults); correctness/fee-bleed, not edge |
...
| [0042 § 9 M51 amendment](0042-paper-exploration-profile.md#9-m51-amendment-2026-07-02--paper-only-per-coin-liquidity-relax-paper_relax_per_coin_liquidity) | M51 — paper-only per-coin liquidity relax `PAPER_RELAX_PER_COIN_LIQUIDITY` (depth > $2,500, spread ≤ 0.30%, `paper`+flag only) |
...
| [0045](0045-m38-fill-time-tp-rebase-and-fill-acceptance-guard.md) | M38 — Fill-time TP rebase + fill-acceptance guard (D1/D2); M47 Option-B no-rebase amendment; M48 fill-anchored geometry-integrity leg (`DEGENERATE_GEOMETRY_AT_FILL`) |
