...
| **[MEDIUM] Per-tier book-depth floors — calibration ongoing.** | `COIN_DEPTH_FLOOR_10BPS_USDT` in `apps/engine/src/risk/const/riskConsts.ts` | M22 | M22 recalibrated from M19's conservative round numbers `{t1:20k,t2:10k,t3:5k}` to book-consumption-ratio-anchored values `{t1:10k,t2:2.5k,t3:2k}` (one-sided at `MAX_EXPOSURE_PER_COIN_USDT = 250`). Evidence base: one calm day (10 rejects, 2026-06-04); 7 unblocked at $3,468–$9,174; 3 still blocked at $529/$681/$2,321. The floors are a correction, not a proven optimum. 14-day post-deploy realized-vs-modeled slippage telemetry (ADR 0004 §6a) must validate before any scale-up. This entry closes when real fills confirm the floors. (M19 → M22) |
...
| Market breadth extreme-threshold empirical validation | `STRESS_BREADTH_DISTANCE_PCT=40` in `apps/engine/src/risk/const/riskConsts.ts` | June 3 hotfix | Threshold raised 30→40 post-soak analysis: 18–21 bars/day at 80%+ breadth (routine) vs 1 bar/5-days at 90%+ (extreme). New threshold (≤10% or ≥90%) confirmed live on 2026-06-03; set empirically from 5-day window. Should re-evaluate after longer soak window (target 30–60 days) to confirm 90% is the right extreme for altcoin USDT-M breadth distribution. |
