...
export class AuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (this.isFailure(verified)) {
            // M11a W1.5 — BAD_SIGNATURE rides on the wire enum directly; audit
            // / metric routing keys off `verified.reason === BAD_SIGNATURE`.
            if (verified.reason === AuthFailureReasonEnum.BAD_SIGNATURE) {
                this.logger.warn(`auth.denied reason=${verified.reason} signatureMismatch=true`);
            }
