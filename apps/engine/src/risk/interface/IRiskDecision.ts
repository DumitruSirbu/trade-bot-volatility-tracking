...
export interface IRiskDecision {
    ...
    // M27 observability-only. The classified market_stress halt leg (verbatim copy of the
    // string the gate persists to risk_state.halt_reason) on a stress reject — fresh or
    // already-halted-day. null for every non-stress outcome (approval, strategy skip, and all
    // other reject reasons). The orchestrator stamps decisions.halt_reason_detail from this
    // field directly — it never re-derives the leg.
