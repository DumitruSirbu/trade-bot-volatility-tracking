...
describe('no-5433 soak-DB tripwire', () => {
    ...
    it('has no :5433 URL literals in test files', () => {
        ...
        const result = execSync(
            `grep -rEn ':5433([^0-9]|$)' "${testsDir}" --include="*.ts" || true`,
        ).toString();
        ...
        if (disallowedLines.length > 0) {
            throw new Error(
                'Found :5433 URL literals in test files (only validateEnv.spec.ts is allowed):\n' +
    ...
    it('has no DB_PORT=5433 instructions in test files', () => {
        const result = execSync(
            `grep -rn 'DB_PORT=5433' "${testsDir}" --include="*.ts" || true`,
        ).toString();
        ...
        if (matches.length > 0) {
            throw new Error(
                'Found stale DB_PORT=5433 instructions in test files:\n' + matches.join('\n'),
            );
        }
