...
export const simulatedFillSchema = z
    .object({
    ...
    .superRefine((fill, ctx) => {
        if (fill.missed && !fill.missedReason) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: 'missed=true requires missedReason',
                path: ['missedReason'],
            });
        }
        if (!fill.missed && !fill.closeReason) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: 'missed=false requires closeReason',
                path: ['closeReason'],
            });
        }
        if (!fill.missed && !fill.exitPrice) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: 'missed=false requires exitPrice',
                path: ['exitPrice'],
            });
        }
