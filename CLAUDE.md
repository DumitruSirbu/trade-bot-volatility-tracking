...
**Current status:** M27 done, M28 next — full milestone history, test counts, and go-live gates in `docs/milestone-log.md`.

**M27 — Decision data-capture completeness (DONE):** Observability-only, no gate path altered (A7 invariant). Added 8 nullable trade-geometry columns to `decisions` (gate_allowed, trade_side, stop_loss, take_profit, qty, notional, leverage, halt_reason_detail) via additive nullable migration. Added `event_id`/`mid_at_trigger` to `book_snapshots` via additive nullable migration + partial UNIQUE index. Live `onVolatilityDetected` book-snapshot writer (best-effort, swallows all errors). `haltReasonDetail` threaded verbatim from gate output, never re-derived. Real `active_positions_count` threaded from `buildGateContext` (eliminates second `findOpen()` DB call). Zod hard-fail in TEST env. `MissedReasonEnum` exported from shared package; shadow `deriveMissedReason` uses typed enum. 64 new tests, 3,137 total green. Review: 1 round — zero blockers, zero highs, zero mediums at close. Smoke: engine healthy, zero errors/warnings 10h post-deploy. **Post-deploy monitoring:** 24–48h confirm `decisions` geometry columns populating; watch `book_snapshots` for non-null `event_id` rows; check `halt_reason_detail` on any stress reject.
