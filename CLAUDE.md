...
**Pre-M9 deferred:** M8 W6.1 robustness re-runs (criteria 7+9), M2 partition-rollover (carry if volume fresh), CLI auth (M11), eventAnchoredVwap (M11), depth-aware slippage (M9), politisWhite comment precision (cosmetic).
**Next:** **M9 — Observability & control** (live hot-reload signaling, metric dashboards, operator CLI / log surfaces).
