...
**Next:** **M11a — Local soak hardening** (Binance demo-trading migration, auth rotation, secrets in `chmod 600` `.env`, bind policy, nightly backups + offsite + restore-test, disk/retention sanity, soak runbook, crash-recovery drill, restricted v1 profile). $0 infra; gates **M11b — Cloud go-live & scaling** (cheap-VPS / hyperscaler-VM / managed profiles, secrets manager, static egress IP, single-writer guard, external reverse proxy). M11 split rationale + soak exit criteria in `docs/plans/M11-go-live-hardening.md`.
