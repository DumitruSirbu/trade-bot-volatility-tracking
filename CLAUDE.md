...
**Pre-M15 deferred items** (carried from prior milestone close logs, plus items from M13 and post-M13 smoke):
...
**Next:** **M14 — CI review gate** (review agents + QA agent + scribe agent wired into CI on PRs; supply-chain gate for dependencies; block merge on critical findings).
