...
**Pre-M8 deferred:** M2 partition-rollover task for fresh Postgres volume (narrow scope, test-harness or service pre-create partitions for current/upcoming date window).
**Next:** **M8 — Live trading validation & strategy comparison** (strategy comparison on same event_id, depth-aware slippage, walk-forward splits).
